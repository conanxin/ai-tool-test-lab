...
- **阶段**：ATL-EVOMAP-3B + ATL-EVOMAP-3A + ATL-EVOMAP-2 + ATL-EVOMAP-1 + ATL-RESUME-2C — Castform vendor fix confirmed + EvoMap Phase 1 smoke + Phase 2 session-context + Phase 3a skill distillation + Phase 3b signal detector · Castform final_status `VENDOR_FIX_CONFIRMED_BY_RETEST`; EvoMap Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED`; EvoMap Phase 2 `SESSION_CONTEXT_TEST_PARTIAL`; EvoMap Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED`; EvoMap Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL`
- **目标**：ATL-EVOMAP-3B 收口 — ATL-EVOMAP-3B EvoMap Evolver OpenClaw-specific signal detector 已完成（PARTIAL）；detector (scripts/openclaw_signal_detector.py, stdlib only) 在 fixture 上 emit 5/5 qualified signals（tool_bypass:exec-on-grep、repeated_tool_usage:exec、protocol_drift:wrong-tool-for-file-read、session_context:openclaw、repo_context:ai-tool-test-lab），对真实 Phase 2 evolver run output emit 3/5（符合预期）；manual signal injection 路径打通（detector → memory_graph.jsonl → evolver run scanner），scanner 确实读取；根因发现：evolver scanner strips qualified keys to bare form (tool_bypass:exec-on-grep → tool_bypass)，导致 selector 选 pre-existing `gene_tool_integrity` (local bank) 而非新 Gene `gene_distilled_openclaw-tool-use-discipline`；Phase 3C HOLD — 需先修 selector 或让 new Gene signals_match 用 bare form；安全边界全部 PASS（no Hub / no publish / no validator / no --loop / no credits / no source modification）；未执行 `--approve` / `solidify`；Castform ATL-RESUME-2C 已完成；ATL-EVOMAP-1/2/3A 已完成
...
- **第二个案例**：[EvoMap Evolver — OpenClaw Local Self-Evolution Smoke Test v0](cases/evomap-evolver-openclaw-v0/) — Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED` + Phase 2 `SESSION_CONTEXT_TEST_PARTIAL` + Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED` + Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL`; Phase 1：evolver 不是通用 log analyzer；Phase 2：evolver 能扫描 session 但 signal 泛化；Phase 3a：手工建立 OpenClaw-specific Gene 真实落盘；Phase 3b：detector + injection 路径打通，selector 选 pre-existing gene（qualified→bare strip 是 root cause），需 Phase 3C 修 selector
