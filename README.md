...
- **阶段**：ATL-EVOMAP-3C + ATL-EVOMAP-3B2 + ATL-EVOMAP-3B + ATL-EVOMAP-3A + ATL-EVOMAP-2 + ATL-EVOMAP-1 + ATL-RESUME-2C — Castform vendor fix confirmed + EvoMap Phase 1 smoke + Phase 2 session-context + Phase 3a skill distillation + Phase 3b signal detector + Phase 3b2 bare signal compatibility + Phase 3c solidify · Castform final_status `VENDOR_FIX_CONFIRMED_BY_RETEST`; EvoMap Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED`; EvoMap Phase 2 `SESSION_CONTEXT_TEST_PARTIAL`; EvoMap Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED`; EvoMap Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL`; EvoMap Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED`; EvoMap Phase 3c `OPENCLAW_SOLIDIFY_PARTIAL`
- **目标**：ATL-EVOMAP-3C 收口 — ATL-EVOMAP-3C EvoMap Evolver OpenClaw solidify 已完成（**PARTIAL**，4 PASS + 1 PARTIAL）；pre-approve review 确认 pending run run_1781793744810 + selected Gene gene_distilled_openclaw-tool-use-discipline-bare-compatible（正确）；`evolver review --approve` 成功执行并 auto-triggered solidify；Evolver 自身 HOLLOW COMMIT detection 触发（diff 只含 GEP assets/metadata，无真实代码变更），auto-rollback via git stash（3 次，3 个 ref）；3 个手动 solidify 尝试 (evolver solidify / node index.js solidify) 都触发 HOLLOW COMMIT detection；3 EvolutionEvents 生成 (evt_1781795571190 → evt_1781795618207 → evt_1781795639960 3-level parent chain)；3 ValidationReports 也生成 (vr_1781795568895, vr_1781795617822, vr_1781795638599)；0 Capsule 生成 (capsule_count=0) — HOLLOW COMMIT detection 阻止空 commit；all 15 hard boundaries respected（no Hub / no auto-publish / no validator / no --loop / no credits / no source modification）；2 个 untracked files preserved via git stash pop（无数据丢失）；Evolver HOLLOW COMMIT safety net 自动触发（这是 evolver v1.89.14 的内置安全机制，不是用户强制）；Phase 3C-V2 (待用户指令) 可用真实代码变更触发 non-hollow solidify 验证 Capsule 创建路径；Phase 4 cross-session reuse test 待 Phase 3C-V2 完成；Castform ATL-RESUME-2C 已完成；ATL-EVOMAP-1/2/3A/3B/3B2 已完成
...
- **第二个案例**：[EvoMap Evolver — OpenClaw Local Self-Evolution Smoke Test v0](cases/evomap-evolver-openclaw-v0/) — Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED` + Phase 2 `SESSION_CONTEXT_TEST_PARTIAL` + Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED` + Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL` + Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED` + Phase 3c `OPENCLAW_SOLIDIFY_PARTIAL`; Phase 1：evolver 不是通用 log analyzer；Phase 2：evolver 能扫描 session 但 signal 泛化；Phase 3a：手工建立 OpenClaw-specific Gene 真实落盘；Phase 3b：detector + injection 路径打通，selector 选 pre-existing gene（qualified→bare strip 是 root cause）；Phase 3b2：bare-signal compat 解决 selector match 问题（first full PASS phase）；Phase 3c：approve/solidify 流程完整验证，HOLLOW COMMIT detection 触发，3 EvolutionEvents 生成，0 Capsule（safety net 工作正常）
