...
- **阶段**：ATL-EVOMAP-3B2 + ATL-EVOMAP-3B + ATL-EVOMAP-3A + ATL-EVOMAP-2 + ATL-EVOMAP-1 + ATL-RESUME-2C — Castform vendor fix confirmed + EvoMap Phase 1 smoke + Phase 2 session-context + Phase 3a skill distillation + Phase 3b signal detector + Phase 3b2 bare signal compatibility · Castform final_status `VENDOR_FIX_CONFIRMED_BY_RETEST`; EvoMap Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED`; EvoMap Phase 2 `SESSION_CONTEXT_TEST_PARTIAL`; EvoMap Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED`; EvoMap Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL`; EvoMap Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED`
- **目标**：ATL-EVOMAP-3B2 收口 — ATL-EVOMAP-3B2 EvoMap Evolver bare-signal compatibility 已完成（**全 PASS**，ATL-EVOMAP 系列第一个 4 维度全过的 phase）；创建 gene_distilled_openclaw-tool-use-discipline-bare-compatible (10 signals_match = 5 bare + 5 qualified) 并安装到 runtime .evolver/gep/genes.json (12→13 genes)；注入 5 个 bare signal MemoryGraphEvent 到 memory/evolution/memory_graph.jsonl；evolver run 输出 `[Signals] Multi-strategy: ... | score-only: tool_bypass`（5 bare signals 合并为单个 bare）；Selector 选中新 Gene (selected_gene_id == gene_distilled_openclaw-tool-use-discipline-bare-compatible)，Last-run gene gene_tool_integrity 输给新 bare-compatible Gene；pending run run_1781793744810 可被 approve/solidify；Phase 3C UNBLOCKED — 等用户明确指令；安全边界全部 PASS（no Hub / no auto-publish / no validator / no --loop / no credits / no source modification）；未执行 `--approve` / `solidify`；Castform ATL-RESUME-2C 已完成；ATL-EVOMAP-1/2/3A/3B 已完成
...
- **第二个案例**：[EvoMap Evolver — OpenClaw Local Self-Evolution Smoke Test v0](cases/evomap-evolver-openclaw-v0/) — Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED` + Phase 2 `SESSION_CONTEXT_TEST_PARTIAL` + Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED` + Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL` + Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED`; Phase 1：evolver 不是通用 log analyzer；Phase 2：evolver 能扫描 session 但 signal 泛化；Phase 3a：手工建立 OpenClaw-specific Gene 真实落盘；Phase 3b：detector + injection 路径打通，selector 选 pre-existing gene（qualified→bare strip 是 root cause）；Phase 3b2：bare-signal compat 解决 selector match 问题（first full PASS phase）
