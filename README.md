...
- **阶段**：ATL-EVOMAP-6D + ATL-EVOMAP-3C-V2 + ATL-EVOMAP-3C + ATL-EVOMAP-3B2 + ATL-EVOMAP-3B + ATL-EVOMAP-3A + ATL-EVOMAP-2 + ATL-EVOMAP-1 + ATL-RESUME-2C — Castform vendor fix confirmed + EvoMap Phase 1 smoke + Phase 2 session-context + Phase 3a skill distillation + Phase 3b signal detector + Phase 3b2 bare signal compatibility + Phase 3c solidify + Phase 3c-v2 non-hollow solidify · Castform final_status `VENDOR_FIX_CONFIRMED_BY_RETEST`; EvoMap Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED`; EvoMap Phase 2 `SESSION_CONTEXT_TEST_PARTIAL`; EvoMap Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED`; EvoMap Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL`; EvoMap Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED`; EvoMap Phase 3c `OPENCLAW_SOLIDIFY_PARTIAL`; EvoMap Phase 3c-v2 `NON_HOLLOW_SOLIDIFY_BLOCKED`
- **目标**：ATL-EVOMAP-3C-V2 收口 — ATL-EVOMAP-3C-V2 EvoMap Evolver non-hollow solidify 已完成（**BLOCKED**，1 PASS + 1 FAIL-BLOCKED + 2 NOT-EXECUTED + 1 PASS）；real code diff in place - scripts/openclaw_tool_use_fixture.py (3.4 KB, stdlib-only) + fixture (447 B)；python3 scripts/openclaw_tool_use_fixture.py runs successfully, output valid JSON (exec_count=3, read_count=2, edit_count=2, exec_ratio=0.375, has_session_context=true, has_repo_context=true)；evolver review 确认 untracked files 包含 real code；13 evolver run cycles (4-16) - ALL picked GEP-internal Genes (gene_gep_repair_from_errors / gene_gep_innovate_from_opportunity), NEVER picked gene_distilled_openclaw-tool-use-discipline-bare-compatible；Per 硬边界 #12/#13 BLOCKED - selected Gene ≠ OpenClaw Gene, no approve executed；capsule_count = 0 (correct given no approve)；Selector root cause: evolver selector 是 history-and-session driven；consecutive_failure_streak_3 feedback loop from Phase 3C failed events biases selector toward gene_gep_repair_from_errors；LLM context pollution - evolver scan reads my own message text (make our gene have the highest priority) and interprets as user_feature_request, biases toward gene_gep_innovate_from_opportunity；memory graph injection cannot override GEP internal state；no run --gene= flag, only experiment --gene= supports gene override；Phase 3B2 PASS remains valid (verified in clean environment)；Phase 3C-V2 BLOCKED is legitimate - surfaces selector reproducibility concern；Phase 4 recommended path: Phase 4A isolation test (copy .evolver to fresh dir, new session, verify selector)；Phase 4B capsule creation test (in isolation env)；Phase 4C cross-session reuse (two sessions share events.jsonl)；all 15 hard boundaries respected (no Hub / no auto-publish / no validator / no --loop / no credits / no source modification / no secrets)；Real code diff 是合法 ai-tool-test-lab 文件（per 硬边界 #12）；Castform ATL-RESUME-2C 已完成；ATL-EVOMAP-1/2/3A/3B/3B2/3C 已完成
...
- **第二个案例**：[EvoMap Evolver — OpenClaw Local Self-Evolution Smoke Test v0](cases/evomap-evolver-openclaw-v0/) — Phase 1 `LOCAL_OFFLINE_SMOKE_COMPLETED` + Phase 2 `SESSION_CONTEXT_TEST_PARTIAL` + Phase 3a `OPENCLAW_SKILL_DISTILLATION_COMPLETED` + Phase 3b `OPENCLAW_SIGNAL_DETECTOR_PARTIAL` + Phase 3b2 `BARE_SIGNAL_COMPATIBILITY_COMPLETED` + Phase 3c `OPENCLAW_SOLIDIFY_PARTIAL` + Phase 3c-v2 `NON_HOLLOW_SOLIDIFY_BLOCKED`; Phase 1：evolver 不是通用 log analyzer；Phase 2：evolver 能扫描 session 但 signal 泛化；Phase 3a：手工建立 OpenClaw-specific Gene 真实落盘；Phase 3b：detector + injection 路径打通，selector 选 pre-existing gene（qualified→bare strip 是 root cause）；Phase 3b2：bare-signal compat 解决 selector match 问题（first full PASS phase）；Phase 3c：approve/solidify 流程完整验证，HOLLOW COMMIT detection 触发，3 EvolutionEvents 生成，0 Capsule（safety net 工作正常）；Phase 3c-v2：real code diff + fixture in place，但 selector 13 cycles 都选 GEP-internal Gene (gene_gep_repair_from_errors / gene_gep_innovate_from_opportunity)，未选 OpenClaw Gene (per 硬边界 #12/#13 BLOCKED)，surfaces evolver history-and-session driven selector 机制
- **EvoMap Local Evolution Kit 子阶段**：5 `LOCAL_EVOLUTION_KIT_COMPLETED` + 6A `HERMES_SYSTEMD_BUNDLE_PASS` + 6B `TELEGRAM_ROUTER_BUNDLE_PASS` + 6C `CODEX_TEST_FAILURE_BUNDLE_PASS` + 7A `DOMAIN_SIGNAL_INJECTION_PASS` + 7B `CROSS_BUNDLE_REGRESSION_PASS` + 8A `NIGHTLY_VALIDATION_LOOP_PASS` + 9A `BUNDLE_CURATOR_SKILL_PASS` + **ATL-EVOMAP-9B `CURATOR_NIGHTLY_INTEGRATION_SMOKE_PASS`** + **ATL-EVOMAP-6D `BROWSER_CONTROL_BUNDLE_PASS`**。Phase 9B 把 Phase 9A 的 curator-generated draft bundle (`sample-safe-bundle-phase9a`) 接入 Phase 8A nightly validation runner 作为 **non-blocking canary lane**；canonical 4 bundles 仍是 blocking 链，canary 失败不致 overall_status FAIL；runner 现跑 7 phase validators(5/6A/6B/6C/7A/7B/9A),9 个 blocking checks 全 PASS;**Phase 6D** 在 5/6A/6B/6C 之后追加第 5 个 canonical portable bundle (`browser-control-recovery`): 覆盖 OpenClaw browser-control 失败模式(端口/auth/launch/navigation/screenshot/anti-curl-bypass),仍保持 9B canary lane 不变; nightly canonical blocking lane 从 4 升 5; 8A/9B validators 已做 forward-compatible 修复(不改 hardcode 4、不降 secret/report/artifact checks); runner `_resolve_bundle_paths` 改为 manifest-driven(原 4 仍作 fallback); 详见 [cases/evomap-evolver-openclaw-v0/phase9b-curator-nightly-integration/README.md](cases/evomap-evolver-openclaw-v0/phase9b-curator-nightly-integration/README.md) + [reports/ATL_EVOMAP_9B_CURATOR_NIGHTLY_INTEGRATION_REPORT.md](reports/ATL_EVOMAP_9B_CURATOR_NIGHTLY_INTEGRATION_REPORT.md) + [cases/evomap-evolver-openclaw-v0/phase6d-browser-control-bundle/README.md](cases/evomap-evolver-openclaw-v0/phase6d-browser-control-bundle/README.md) + [reports/ATL_EVOMAP_6D_BROWSER_CONTROL_BUNDLE_REPORT.md](reports/ATL_EVOMAP_6D_BROWSER_CONTROL_BUNDLE_REPORT.md)
