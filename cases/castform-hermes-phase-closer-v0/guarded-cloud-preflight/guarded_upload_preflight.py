#!/usr/bin/env python3
"""
ATL-4C: guarded_upload_preflight.py

Guarded upload preflight for Castform cloud smoke run.

- Default behavior: REFUSE the upload, print the blocked banner, exit 1.
...
- Does not import or call upload_training_run.
...
The ONLY way to actually upload is for the user to explicitly satisfy ALL
pre-launch gates in FINAL_LAUNCH_GATE.md AND flip
actual_upload_allowed_in_this_phase to true in a future phase (ATL-5).
"""
...
def banner() -> None:
    print("ATL-4C guarded upload preflight")
    print("actual_upload_allowed_in_this_phase=false")
...
def main() -> int:
    ...
    print("[INFO] ATL-4C guarded_upload_preflight.py")
    cfg = load_config()

    # Config-level gate
    if cfg.get("actual_upload_allowed_in_this_phase") is not True:
        print("[GUARD] actual_upload_allowed_in_this_phase is false (ATL-4C default)")
    else:
        print("[INFO] actual_upload_allowed_in_this_phase is true")
    ...
    if cfg.get("actual_upload_allowed_in_this_phase") is not True:
        banner()
        print("[GUARD] upload refused (ATL-4C default — actual_upload_allowed_in_this_phase=false)")
        return 1

    # The branch below is intentionally unreachable in ATL-4C.
    # A future phase (ATL-5, after explicit user authorization) may flip
    # actual_upload_allowed_in_this_phase to true.
    print("[FAIL] actual_upload_allowed_in_this_phase=true reached but no upload path is wired in ATL-4C", file=sys.stderr)
    return 1
